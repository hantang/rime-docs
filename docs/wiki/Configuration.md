...
Rime 所使用的配置文件在 [用戶文件夾](./UserData.md) 及 [共享文件夾](./SharedData.md)。
...
配置源文件的位置詳見 [用戶文件夾](./UserData.md) 及 [共享文件夾](./SharedData.md)。
...
部署後出現錯誤，請查看 `INFO` 日誌（[參考](./RimeWithSchemata.md#關於調試)），
