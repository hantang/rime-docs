...
[《定製指南》|CustomizationGuide](./CustomizationGuide.md#定製指南)提供了[模糊音](./CustomizationGuide.md#模糊音)定製方法及代碼模板。
...
實現雙拼的關鍵，是用 Rime 的[「拼寫運算」](./SpellingAlgebra.md)技術建立雙拼輸入碼到全拼的映射表，從而對接朙月拼音詞典。在[《輸入方案設計書》](./RimeWithSchemata.md)中有一個完整的代碼示例。
