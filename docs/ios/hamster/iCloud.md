...
- 4、仓App内Rime目录下有方案部署使用后，实时记录用户词频的`xxx.userdb`文件夹，iCloud里面不要有`xxx.userdb`文件夹（可通过正则过滤`^.*[.]userdb.*$，^.*build.*$`文件夹，点击“拷贝至iCloud”时，就不会将仓内的`xxx.userdb`文件夹拷贝到iCloud），部署就不会覆盖仓内部新的用户词频文件。
